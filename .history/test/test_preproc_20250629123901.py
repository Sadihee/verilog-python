...
from verilog_python import Preproc  # Replace 'your_module' with the actual module name
...
class TestPreproc(unittest.TestCase):
    ...
    def test_resolve_include(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            # Create a temporary include file within the temporary directory
            temp_file_path = os.path.join(temp_dir, "included.v")
            with open(temp_file_path, 'w') as f:
                f.write("module included(input a, output b);\nendmodule")

            # Add the temporary directory to include paths
            self.preproc.include_paths.append(temp_dir)

            # Resolve the include file
            resolved_path = self.preproc._resolve_include("included.v", os.path.join(temp_dir, "test_file.v"))
            print("***", resolved_path)  # Debugging output
